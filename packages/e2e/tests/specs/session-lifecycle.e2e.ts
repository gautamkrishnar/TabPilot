...
describe('Session lifecycle', () => {
  ...
  it('host starts session and participant receives navigation events', async () => {
    ...
    await browser.waitUntil(
      ...
      { timeout: 60_000, timeoutMsg: 'Participant did not appear in host dashboard' },
    );
    ...
    await browser.waitUntil(
      ...
      { timeout: 60_000, timeoutMsg: 'Participant did not receive first NAVIGATE_TO event' },
    );
    ...
    await browser.waitUntil(
      ...
      { timeout: 60_000, timeoutMsg: 'Participant did not receive second NAVIGATE_TO event' },
    );
  ...
  it('host can lock the session to prevent new participants from joining', async () => {
    ...
    await browser.waitUntil(
      ...
      { timeout: 60_000, timeoutMsg: 'Expected locked session indicator on join page' },
    );
...
describe('Session lifecycle — voting flow', () => {
  ...
  it('participant can submit a vote and host can reveal it', async () => {
    ...
    await browser.waitUntil(
      ...
      { timeout: 60_000, timeoutMsg: 'Revealed votes did not appear on participant view' },
    );
  });
});
